...
void setup() {
  ...
  adc.setAveraging(4);
  adc.setResolution(16); // call this before the compare or continuous functions!!
  //adc.enableCompare(1.0/6.6*adc.getMaxValue(), 0); // true if value < 1.0V
  adc.enableCompareRange(-1.0*adc.getMaxValue()/3.3, 2.0*adc.getMaxValue()/3.3, 0, 1);
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    } else if(c=='r') { // restart conversions
