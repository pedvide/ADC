/*
    This example shows how to use the IntervalTimer library and the ADC library
   in the Teensy 3.0/3.1

    The three important objects are: the ADC, the (one or more) IntervalTimer
   and the same number of RingBuffer.

    The ADC sets up the internal ADC, you can change the settings as usual.
   Enable interrupts. The IntervalTimer (timerX) executes a function every
   'period' microseconds. This function, timerX_callback, starts the desired ADC
   measurement, for example: startSingleRead, startSingleDifferential,
   startSynchronizedSingleRead, startSynchronizedSingleDifferential you can use
   the ADC0 or ADC1 (only for Teensy 3.1).

    When the measurement is done, the adc_isr is executed:
        - If you have more than one timer per ADC module you need to know which
   pin was measured.
        - Then you store/process the data
        - Finally, if you have more than one timer you can check whether the
   last measurement interruped a a previous one (using adc->adcX->adcWasInUse),
   and restart it if so. The settings of the interrupted measurement are stored
   in the adc->adcX->adc_config struct.
...
void setup() {
  ...
  // wait enough time for the first timer conversion to finish (depends on
  // resolution and averaging), with 16 averages, 12 bits, and ADC_MED_SPEED in
  // both sampling and conversion speeds it takes about 36 us.
  delayMicroseconds(
      25); // if we wait less than 36us the timer1 will interrupt the conversion
  ...
  // Timer0 starts a comversion and 25 us later timer1 starts a new one,
  // "pausing" the first, about 36 us later timer1's conversion is done, and
  // timer0's is restarted, 36 us later timer0's conversion finishes. About 14
  // us later timer0 starts a new conversion again. (times don't add up to 120
  // us because the timer_callbacks and adc_isr take time to execute, about 2.5
  // us and 1 us, respectively) so in the worst case timer0 gets a new value in
  // about twice as long as it would take alone. if you change the periods, make
  // sure you don't go into a loop, with the timers always interrupting each
  // other
...
void printTimedADCInfo(uint8_t adc_num, uint16_t *buffer,
                       uint32_t &delta_time) {
  ...
  Serial.printf("ADC:%d delta time:%d freq:%d - min:%d max:%d avg:%d rms:%d\n",
                adc_num, delta_time, (1000 * BUFFER_SIZE) / delta_time,
...
void timer0_callback(void) {
  ...
  adc->adc0->startSingleRead(
      readPin0); // also: startSingleDifferential, analogSynchronizedRead,
...
// first: see which pin finished and then save the measurement into the correct
// buffer
void adc0_isr() {

#if defined(__IMXRT1062__) // Teensy 4.0
  uint8_t pin =
      ADC::sc1a2channelADC0[ADC1_HC0 &
                            0x1f]; // the bits 0-4 of ADC0_SC1A have the channel
#else
  uint8_t pin =
      ADC::sc1a2channelADC0[ADC0_SC1A &
                            ADC_SC1A_CHANNELS]; // the bits 0-4 of ADC0_SC1A
                                                // have the channel
#endif
  ...
  if (pin == readPin0) {
    ...
    if (buffer_0_count < BUFFER_SIZE) {
      ...
      if (buffer_0_count == BUFFER_SIZE)
        delta_time_0 = timed_read_elapsed;
    }
    ...
    if (buffer_1_count < BUFFER_SIZE) {
      ...
      if (buffer_1_count == BUFFER_SIZE)
        delta_time_1 = timed_read_elapsed;
    }
