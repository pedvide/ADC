/* Example for triggering the ADC with Timer using DMA instead of interrupts
    ...
    This example is pretty much the same as adc_timer_dma.ino except only one
  buffer is added to each of our DMA buffer objects, so instead of running
  continuously in round robin format it stops after each buffer is filled and
  waits for us to signal to run again.  We do that by entering something at the
  keyboard.
...
#endif
  Serial.println("After enableDMA");
  Serial.flush();
