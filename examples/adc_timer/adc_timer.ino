/* Example for triggering the ADC with Timer
    ...
    On Teensy 3.x this uses the PDB timer, and should work the same as the
   example adc_pdb.ino, but has been extended some to add additional command to
   read in a whole buffer of readings, using the timer.
...
void loop() {
    ...
    if (c == 'v') { // value
...
#endif
    } else if (c == 's') { // start Timer, before pressing enter write the
                           // frequency in Hz
      ...
      if (freq == 0) {
        ...
        adc->adc0->startSingleRead(
            readPin); // call this to setup everything before the Timer starts,
        ...
        adc->adc1->startSingleRead(
            readPin2); // call this to setup everything before the Timer starts
  ...
  if (delta_time_adc_0)
    printTimedADCInfo(ADC_0, buffer_ADC_0, delta_time_adc_0);
  if (delta_time_adc_1)
    printTimedADCInfo(ADC_1, buffer_ADC_1, delta_time_adc_1);
